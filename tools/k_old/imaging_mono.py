...
parser = argparse.ArgumentParser(description='Processing Su method',
                                 usage='cd gprMax; python -m tools.imaging_mono jsonfile velocity_structure -plot')
...
def calc_Amp(z, x, i): # i: 0~antenna_num-1を入力する
    ...
    if tau_index[i] < len(data_list[i-1]):
        Amp_array[i] = data_list[i-1][tau_index[i], i]
    elif tau_index[i] >= len(data_list[i-1]):
        Amp_array[i] = 0
...
def calc_corr():
    ...
    for x in tqdm(range(imaging_grid_x), desc='calculating cross-correlation'):
        for z in range(imaging_grid_z):
            ...
            cross_corr[z, x] = np.array([calc_Amp(z, x, rx) for rx in range(antenna_num)])
...
if args.plot == False:
    ...
    if args.velocity_structure == 'n':
        ...
        if args.theory == True:
            ...
            np.savetxt(output_dir_path + '/imaging_result_mono.csv', corr, delimiter=',')
...
if args.velocity_structure == 'n':
    ...
    if args.theory == True:
        ...
        plt.savefig(output_dir_path + '/imaging_result_mono.png')
plt.show()