...
output_basename = 'background_subtracted'
...
# 背景除去
subtracted_data = outputdata[:, 1:] - outputdata_ave[:, np.newaxis] # 1列目はpml内なので除外
print(f"Calculated background-subtracted data with shape {subtracted_data.shape}")

# 背景除去後の平均強度
subtracted_data_ave = np.mean(subtracted_data, axis=1)
subtracted_data_ave_db = 20 * np.log10(np.abs(subtracted_data_ave) / np.amax(np.abs(subtracted_data_ave)) + 1e-12) # avoid log(0)

# plot background only
plt.figure(figsize=(4, 8), facecolor='w', edgecolor='w')
plt.plot(subtracted_data_ave_db, np.arange(subtracted_data.shape[0]) * dt * 1e9) # time in ns
plt.xlabel('Amplitude (dB)', size=14)
plt.ylabel('Time (ns)', size=14)
plt.ylim(subtracted_data.shape[0] * dt * 1e9, 0) # time in ns
...
im = ax[0].imshow(subtracted_data,
             extent=[0, subtracted_data.shape[1] * rx_step, outputdata.shape[0] * dt * 1e9, 0],
            interpolation='nearest', aspect='auto', cmap='seismic',
            vmin=-np.amax(np.abs(subtracted_data))/1e3, vmax=np.amax(np.abs(subtracted_data))/1e3)
...
ax[1].plot(subtracted_data_ave_db, np.arange(subtracted_data.shape[0]) * dt * 1e9) # time in ns
ax[1].set_xlabel('Amplitude (dB)', size=14)
ax[1].set_ylabel('Time (ns)', size=14)
ax[1].set_ylim(subtracted_data.shape[0] * dt * 1e9, 0) # time in ns
